...
Only minor changes were made to make it compatible with the new codebase.
"""
...
class TrainLoop:
            ...
            if dist.get_rank() == 0:
                ...
                try:
                    ...
                    for key, value in state_dict.items():
                        new_key = key.replace('module.', '')  # remove 'module.' prefix from key
                        new_state_dict[new_key] = value
                    ema_params = self.mp_trainer.state_dict_to_master_params(new_state_dict)
    ...
    def run_loop(self):
        ...
        print("Memory chaced ",th.cuda.memory_cached())
    ...
    def run_validation(self):
        #get the data, assumes data is in 'brats' format
        self.iterdatal = iter(self.datal)
        losses = []
        for batch, cond, label, _, _ in self.iterdatal:
            #run_validation
            avg_lossmse, samples = self.validate(batch, cond)
            print("The loss for this batch was ", avg_lossmse)
            losses.append(avg_lossmse)
    ...
    def validate(self, batch, cond):
        ...
        with th.no_grad():  # Disable gradient calculation
            for i in range(0, batch.shape[0], self.microbatch):
                ...
                validation_losses.append(lossmse)
                samples.append(sample)

        avg_lossmse = th.mean(th.stack(validation_losses))
        return avg_lossmse, samples
...
def log_loss_seg(losses):
    for key, values in losses.items():
        logger.logkv_mean(f"{key}", list(values)[0].cpu().numpy().mean())